...
class CARLA_Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data['junction'] = measurements[self.seq_len-1]['junction']
        data['target_speed'] = measurements[self.seq_len-1]['target_speed']
        data['vehicle_hazard'] = measurements[self.seq_len-1]['vehicle_hazard']
        data['walker_hazard'] = measurements[self.seq_len-1]['walker_hazard']
        data['stop_sign_hazard'] = measurements[self.seq_len-1]['stop_sign_hazard']
        data['angle'] = measurements[self.seq_len-1]['angle']

        data['measurements'] = loaded_measurements
        

        mean_lateral_distance = calculate_mean_lateral_distance(data)
        data['mean_lateral_distance'] = mean_lateral_distance

        target_vehicle_info = get_target_vehicle_distance(labels)
        
        data['distance'] = target_vehicle_info.get('distance', 0)
        #data['distance'] = target_vehicle_info('distance', 0)
        data['target_vehicle_id'] = target_vehicle_info.get('target_vehicle_id', None)
        #data['target_vehicle_id'] = target_vehicle_info('target_vehicle_id', None)
        
        '''
        if target_vehicle_info:
            data['distance'] = process_target_vehicle_info(target_vehicle_info)
        else:
            data['distance'] = 0  # Default value if no target vehicle info
        # Debugging statements
        '''
        '''
        if data['distance'] is None:
            print(f"Index {index}: No target vehicle found. Setting distance to 0.")
        else:
            print(f"Index {index}: Target vehicle ID {data['target_vehicle_id']} at distance {data['distance']}")
        if any(v is None for v in data.values()):
            print(f"Index {index}: Found None in data values, replacing with defaults.")
            return None
        '''
        '''
        # Debugging statements
        if data['distance'] == 0:
            print(f"Index {index}: No target vehicle within distance. Setting distance to 0.")
        else:
            print(f"Index {index}: Target vehicle ID {data['target_vehicle_id']} at distance {data['distance']}")
        '''
        # target points
	...
	#print(f"Index {index}: {data['target_speed']}, {data['distance']}, {data['vehicle_hazard']}")

        return data

def calculate_mean_lateral_distance(data):
        lateral_distances = []
        for measurement in data['measurements']:
            vehicle_x = measurement["x"]
            waypoint_x_positions = [measurement["waypoints"][i][0] for i in range(min(5, len(measurement["waypoints"])))]
            absolute_differences = [abs(vehicle_x - waypoint_x) for waypoint_x in waypoint_x_positions]
            mean_lateral_distance = sum(absolute_differences) / len(absolute_differences)
            lateral_distances.append(mean_lateral_distance)

        mean_lateral_distance = np.mean(lateral_distances) if lateral_distances else 0
        return mean_lateral_distance

def get_target_vehicle_distance(labels):
    # Extract the current vehicle's ego_matrix
    current_vehicle = next((car for car in labels[0] if car['distance'] == -1), None)
    if current_vehicle is None:
        
        return {"target_vehicle_id": None, "distance": 0}
    
    current_ego_matrix = np.array(current_vehicle['ego_matrix'])
    #print("current_ego_matrix:",current_ego_matrix)

    results = []
    
    # Filter the other vehicles based on the conditions
    for car in labels[0]:
        #print("current_car_distance:",car['distance'])
        #print("current_car:",car)
        if car['distance'] == -1:
            #print("new_current_car:",car)
            continue
        #print("current_ego_matrix:",current_ego_matrix)
        ego_matrix = np.array(car['ego_matrix'])
        #print("ego_matrix:",car['ego_matrix'])
        if -current_ego_matrix[1, 3] < -ego_matrix[1, 3]:
            #print(-current_ego_matrix[1, 3], -ego_matrix[1, 3])
            distance_diff = abs(current_ego_matrix[0, 3] - ego_matrix[0, 3])
    ...
    # Select the target vehicle (assuming we choose the one with the smallest distance difference)
    print("results:",results)
    selected_vehicle = max(results, key=lambda x: x["distance"])
    #selected_vehicle = [results]
    print("selected_vehicle:",selected_vehicle)
    return selected_vehicle
    #return results
    '''
    def select_vehicle(results):
        non_zero_distances = [r for r in results if r["distance"] > 0]
        if non_zero_distances:
            return min(non_zero_distances, key=lambda x: x["distance"])
            ...
            return {"target_vehicle_id": 0, "distance": 0}
        '''
        else:
            if results:
                return min(results, key=lambda x: x["distance"])
    ...
    selected_vehicle = select_vehicle(results)
    #print("selected_vehicle:",selected_vehicle)
    return selected_vehicle

    '''
    # Separate results with non-zero and zero distances
    non_zero_results = [r for r in results if r["distance"] != 0]

    # Select the target vehicle
    if non_zero_results:
        selected_vehicle = min(non_zero_results, key=lambda x: x["distance"])
    else:
        selected_vehicle = {"target_vehicle_id": None, "distance": 0}

    return selected_vehicle
    '''
'''
def process_target_vehicle_info(target_vehicle_info):
    distances = [target_vehicle_info.get('distance', 0) for vehicle_info in target_vehicle_info]
    # Process distances as needed, e.g., get the maximum distance
    return min(distances)  # Example: return the maximum distance
'''
def get_depth(data):
    """
...
def lidar_bev_cam_correspondences(world, lidar_vis=None, image_vis=None, step=None, debug=False):
    ...
    if (debug == True):
        ...
        plt.ion()
        plt.imshow(vis_bev)
        plt.savefig(r'/beegfs/.global1/ws/rasa397c-Sankar/transfuser/team_code_transfuser/log/save folder/Visualizations/2/bev_lidar_{}.png'.format(step), bbox_inches='tight')
        plt.close()
        plt.imshow(vis_original_image)
        plt.savefig(r'/beegfs/.global1/ws/rasa397c-Sankar/transfuser/team_code_transfuser/log/save folder/Visualizations/2/image_with_lidar_{}.png'.format(step), bbox_inches='tight')
        plt.close()
        plt.ioff()
